...
export function testAsyncStorage (
  ...
  describe('Standard async storage test', function (): void {
    it('Load state (blank → A)', async function (): Promise<void> {
      ...
      expect(store.state)
        .to.eql(data.blank(), 'The store’s state should be blank right after it’s creation.')

      await ltm.ready

      expect(store.state)
        .to.eql(data.a(), 'The A state should’ve been loaded by this time.')
    })
    ...
    it('Load state (A → B)', async function (): Promise<void> {
      ...
      expect(store.state)
        .to.eql(data.a(), 'The store’s state should be A right after it’s creation.')

      await ltm.ready

      expect(store.state)
        .to.eql(data.b(), 'The B state should’ve been loaded by this time.')
    })
    ...
    it('Load state (A → null)', async function (): Promise<void> {
      ...
      expect(store.state)
        .to.eql(data.a(), 'The store’s state should be unchanged.')
    })
    ...
    it('Save state after mutation', async function (): Promise<void> {
      ...
      expect(await getState(key))
        .to.eql(data.b(), 'The store’s state should’ve been persisted by now.')
    })
  })
}
