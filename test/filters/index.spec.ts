...
describe('Filters', (): void => {
  describe('dummyFilter', function (): void {
    it('Should return true for everything', (): void => {
      ...
      expect(f(payloads.foo)).to.be.true
      expect(f(payloads.fooBar)).to.be.true
    })
  })
  ...
  describe('mutationFiler', function (): void {
    it('Should return false for everything with empty configuration', (): void => {
      ...
      expect(f(payloads.foo)).to.be.false
      expect(f(payloads.fooBar)).to.be.false
    })
    ...
    it('Regexps', (): void => {
      ...
      expect(f(payloads.test)).to.be.true
      expect(f(payloads.bar)).to.be.false
      expect(f(payloads.foo)).to.be.false
      expect(f(payloads.fooBor)).to.be.true
      expect(f(payloads.fooBer)).to.be.true
      expect(f(payloads.fooBar)).to.be.true
    })
  })
})
